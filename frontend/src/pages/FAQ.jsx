      ...
      <div className="section" style={{ paddingTop: 100 }}>
        <div className="container">
          ...
          <div style={{ maxWidth: 700, margin: '0 auto' }}>
            {faqs.map((faq, index) => (
              <div key={index} style={{ marginBottom: 12, background: 'white', borderRadius: 'var(--radius-md)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
                <button
                  onClick={() => setOpenIndex(openIndex === index ? null : index)}
                  style={{ width: '100%', padding: '16px 20px', textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: 600, fontSize: 'var(--font-base)', color: 'var(--dark)', cursor: 'pointer', background: 'none', border: 'none' }}
                >
                  {faq.q}
                  <span style={{ transition: 'transform 0.3s ease', transform: openIndex === index ? 'rotate(180deg)' : 'rotate(0)' }}>▼</span>
                </button>
                {openIndex === index && (
                  <div style={{ padding: '0 20px 16px', color: 'var(--gray)', fontSize: 'var(--font-sm)', lineHeight: 1.7 }}>
                    {faq.a}
                  </div>
                )}
              </div>
